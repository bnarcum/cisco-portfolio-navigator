...
const productLogosDir = path.join(root, "assets/product-logos");

/** Product marks with brand colors (not monochrome Cisco pictograms). */
...
function normalizeProductLogo(raw) {
  let inner = stripSvgWrapper(raw).replace(/<style[\s\S]*?<\/style>/gi, "");
  inner = removeJunkGroups(inner);
  return inner
    .replace(/\sid="[^"]*"/gi, "")
...
function resolveProductLogoPath(rel) {
  const direct = path.join(productLogosDir, rel);
  if (fs.existsSync(direct)) return direct;
  throw new Error(`Missing product logo file: ${rel}`);
}

function svgToSymbol(key, filePath, { preserveColors = false } = {}) {
  const raw = fs.readFileSync(filePath, "utf8");
  const viewBox = raw.match(/viewBox=["']([^"']+)["']/i)?.[1] || "0 0 80 80";
  const inner = preserveColors ? normalizeProductLogo(raw) : normalizeInnerSvg(raw);
...
for (const [key, rel] of Object.entries(PRODUCT_LOGO_MAP)) {
  symbols.push(svgToSymbol(key, resolveProductLogoPath(rel), { preserveColors: true }));
}
